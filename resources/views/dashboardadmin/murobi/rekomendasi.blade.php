...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="progress-header">
            <h1><i class="fas fa-handshake" style="margin-right: 8px;"></i> Murobi - Rekomendasi Ta'aruf</h1>
            <p>Rekomendasikan peserta pria dan wanita yang menurut Anda cocok</p>
        </div>
        ...
        <div class="pairing-card">
            <div class="pairing-card-title">
                <i class="fas fa-bullhorn"></i> Buat Rekomendasi Baru
            </div>

            <form action="{{ route('murobi.rekomendasi.store') }}" method="POST" id="pairingForm">
                @csrf
                ...
                <!-- Submit Button -->
                <button type="submit" class="btn-pasangkan" onclick="return confirm('Apakah Anda yakin ingin merekomendasikan pasangan ini?')">
        ...
        <div class="pairs-table-card">
            <div class="pairs-table-header">
                <h3><i class="fas fa-list"></i> Riwayat Rekomendasi Anda</h3>
            </div>
                    ...
                    <table class="pairs-table">
                        <thead>
                            <tr>
                                ...
                                <th>Pria Direkomendasikan</th>
                                <th>Wanita Direkomendasikan</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="pair-person">
                                            <div class="pair-info">
                                                <div class="pair-name">{{ $rec->nama_pria ?? $rec->karyawan_pria_email }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="pair-person">
                                            <div class="pair-info">
                                                <div class="pair-name">{{ $rec->nama_wanita ?? $rec->karyawan_wanita_email }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>{{ \Carbon\Carbon::parse($rec->created_at)->format('d M Y') }}</td>
                                    <td>
                                        <span class="status-badge {{ $rec->status == 'pending' ? 'status-active' : 'bg-primary text-white' }}">
                                            <i class="fas fa-{{ $rec->status == 'pending' ? 'clock' : 'check' }}"></i> {{ ucfirst($rec->status) }}
                                        </span>
                                    </td>
                                </tr>
                            @endforeach
            ...
            @else
                <div class="empty-pairs">
                    ...
                    <p>Belum ada rekomendasi yang Anda buat</p>
                </div>
            @endif
        </div>
    </div>
